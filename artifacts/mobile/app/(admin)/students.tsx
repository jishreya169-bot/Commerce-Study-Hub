    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.summaryRow, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
        {[
          ...
          { val: Math.round(STUDENTS.reduce((a, s) => a + s.attendance, 0) / STUDENTS.length) + "%", label: "Avg Attend.", color: "#D69E2E" },
        ].map((s) => (
        ...
        {filtered.map((s) => (
          <TouchableOpacity key={s.id} onPress={() => setSelected(s)} style={[styles.studentCard, { backgroundColor: colors.card, borderColor: colors.border, borderLeftColor: s.status === "at-risk" ? "#E53E3E" : colors.border, borderLeftWidth: s.status === "at-risk" ? 4 : 1 }]} activeOpacity={0.85}>
            ...
            <View style={styles.studentInfo}>
              ...
              <Text style={[styles.rollClass, { color: colors.mutedForeground }]}>Roll #{s.roll} • {s.class} • {s.lastActive}</Text>
            ...
            {selected && (
              <>
                ...
                <View style={styles.modalProfile}>
                  ...
                  <Text style={[styles.modalClass, { color: colors.mutedForeground }]}>{selected.class} • Roll #{selected.roll}</Text>
                </View>
                {[
                ...
                <View style={styles.modalActions}>
                  <TouchableOpacity style={[styles.actionBtn, { backgroundColor: "#E53E3E18", borderColor: "#E53E3E30" }]} activeOpacity={0.8} onPress={() => setSelected(null)}>
                    <Text style={[styles.actionBtnText, { color: "#E53E3E" }]}>Suspend</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.actionBtn, { backgroundColor: "#9B7BC418", borderColor: "#9B7BC430" }]} activeOpacity={0.8}>
                    <Text style={[styles.actionBtnText, { color: "#9B7BC4" }]}>Send Message</Text>
                  </TouchableOpacity>
                </View>
              </>
            )}
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { backgroundColor: colors.card }]}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setShowAdd(false)}><Ionicons name="close" size={22} color={colors.mutedForeground} /></TouchableOpacity>
            </View>
            {["Full Name", "Email Address", "Roll Number", "Class"].map((f) => (
            ...
            <TouchableOpacity onPress={() => { setShowAdd(false); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); }} style={[styles.submitBtn, { backgroundColor: "#9B7BC4" }]} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
  ...
  studentName: { flex: 1, fontSize: 13, fontFamily: "Poppins_700Bold" },
